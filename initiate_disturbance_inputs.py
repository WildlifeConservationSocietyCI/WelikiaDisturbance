...
DEM = os.path.join(s.INPUT_DIR_FULL, 'dem', 'WELIKIA_DEM_5m_BURNED_STREAMS_10ft_CLIP.tif')
ECOCOMMUNITIES = os.path.join(s.INPUT_DIR_FULL, 'Welikia_Ecocommunities',
                              'Welikia_Ecocommunities_int.tif')
SITES = os.path.join(s.INPUT_DIR_FULL, 'garden_sites', 'GARDEN_SITES.shp')
BUFFER = os.path.join(s.INPUT_DIR_FULL, 'garden_sites', 'SITE_BUFFER.shp')
TRAILS = os.path.join(s.INPUT_DIR_FULL, 'trails', 'fire_trails.tif')
HUNTING = os.path.join(s.INPUT_DIR_FULL, 'hunting_sites', 'hunting_sites.tif')
REGION_BOUNDARIES = os.path.join(s.INPUT_DIR_FULL, 'region_boundaries', 'disturbance_regions.shp')

# Tabular Inputs, in tables folder
PROXIMITY_RECLASS = os.path.join(s.INPUT_DIR_FULL, 'tables', 'garden', 'proximity_reclass.txt')
SLOPE_RECLASS = os.path.join(s.INPUT_DIR_FULL, 'tables', 'garden', 'slope_reclass.txt')

# The inputs for these are generated by the script
GARDEN_SLOPE_SUITABILITY = os.path.join(s.INPUT_DIR_FULL, 'slope_suitability.tif')
PROXIMITY_SUITABILITY = os.path.join(s.INPUT_DIR_FULL, 'proximity_suitability.tif')
STREAM_SUITABILITY = os.path.join(s.INPUT_DIR_FULL, 'stream_suitability.tif')
...
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'ecocommunities.tif')) is False:
    ...
    ecocommunities_fe.save(os.path.join(s.INPUT_DIR_FULL, 'ecocommunities.tif'))

else:
    ecocommunities_fe = os.path.join(s.INPUT_DIR_FULL, 'ecocommunities.tif')

# dem
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'dem.tif')) is False:
    logging.info('creating ascii dem')
    dem = arcpy.sa.ExtractByMask(DEM, ECOCOMMUNITIES)
    dem.save(os.path.join(s.INPUT_DIR_FULL, 'dem.tif'))
else:
    dem = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'dem.tif'))

# slope
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'slope.tif')) is False:
    logging.info('calculating slope')
    slope = arcpy.sa.Slope(dem, output_measurement='DEGREE')
    slope.save(os.path.join(s.INPUT_DIR_FULL, 'slope.tif'))
else:
    slope = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'slope.tif'))

# aspect
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'aspect.tif')) is False:
    logging.info('creating ascii aspect')
    aspect = arcpy.sa.Aspect(dem)
    slope.save(os.path.join(s.INPUT_DIR_FULL, 'aspect.tif'))
else:
    aspect = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'aspect.tif'))

# flow direction
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'flow_direction.tif')) is False:
    logging.info('creating flow direction raster')
    flow_direction = arcpy.sa.FlowDirection(dem, force_flow='NORMAL')
    flow_direction.save(os.path.join(s.INPUT_DIR_FULL, 'flow_direction.tif'))
else:
    flow_direction = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'flow_direction.tif'))

# garden slope suitability
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'slope_suitability.tif')) is False:
    ...
    slope_suitability.save(os.path.join(s.INPUT_DIR_FULL, 'slope_suitability.tif'))
else:
    slope_suitability = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'slope_suitability.tif'))

# stream suitability
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'stream_suitability.tif')) is False:
    ...
    stream_suitability.save(os.path.join(s.INPUT_DIR_FULL, 'stream_suitability.tif'))
else:
    stream_suitability = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'stream_suitability.tif'))

# proximity suitability
if arcpy.Exists(os.path.join(s.INPUT_DIR_FULL, 'proximity_suitability.tif')) is False:
    ...
    proximity_suitability = arcpy.Raster(os.path.join(s.INPUT_DIR_FULL, 'proximity_suitability.tif'))

arcpy.env.workspace = s.INPUT_DIR_FULL
...
for feature in cursor:
    ...
    print ('region name: {}'.format(feature.BoroName))
    region = str(int(feature.BoroCode))

    #  region output paths
    ecocommunities = os.path.join(s.INPUT_DIR_REGION, '%s_ecocommunities_int.tif' % region)
    reference_raster_region = os.path.join(s.INPUT_DIR_REGION, 'reference_grid_%s.tif' % region)
    reference_ascii_region = os.path.join(s.INPUT_DIR_REGION, 'reference_grid_%s.asc' % region)
    dem_ascii_region = os.path.join(s.INPUT_DIR_REGION, 'fire', 'spatial', region, 'dem.asc')
    aspect_ascii_region = os.path.join(s.INPUT_DIR_REGION, 'fire', 'spatial', region, 'aspect.asc')
    slope_ascii_region = os.path.join(s.INPUT_DIR_REGION, 'fire', 'spatial', region, 'slope.asc')
    trails_region = os.path.join(s.INPUT_DIR_REGION, 'fire', 'spatial', region, 'trails.tif')
    hunting_sites_region = os.path.join(s.INPUT_DIR_REGION, 'fire', 'spatial', region, 'hunting_sites.tif')

    dem_region = os.path.join(s.INPUT_DIR_REGION, 'pond', region, 'dem.tif')
    flow_direction_region = os.path.join(s.INPUT_DIR_REGION, 'pond', region, 'flow_direction.tif')
    stream_suitability_region = os.path.join(s.INPUT_DIR_REGION, 'pond', region, 'stream_suitability.tif')

    slope_suitability_region = os.path.join(s.INPUT_DIR_REGION, 'garden', 'spatial', region, 'slope_suitability.tif')
    proximity_suitability_region = os.path.join(s.INPUT_DIR_REGION, 'garden', 'spatial', region,
                                                'proximity_suitability.tif')
    garden_sites_region = os.path.join(s.INPUT_DIR_REGION, 'garden', 'spatial', region, 'garden_sites.shp')
