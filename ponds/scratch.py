...
class PondDisturbance(s.Disturbance):
    ...
    def assign_pond_locations(self, constraint, num_points, outpath):
        """
    ...
    def dam_points_coordinates(self, points):
        """
    ...
    def create_pond(self, dem, flow_direction, coordinates, temp_point):
        """
    ...
    def calculate_territory(self):
        ...
        landcover_set_null = arcpy.sa.SetNull((self.land_cover == 2) | (self.land_cover == 3), 1)
...
def run(year):
    ...
    if pond_dis.pond_count < pond_dis.CARRYING_CAPACITY:
        ...
        pond_dis.assign_pond_locations(constraint=pond_dis.suitability_points,
                                       num_points=new_ponds,
                                       outpath=pond_dis.TEMP_DIR)

        # convert pond points feature to list of longitude latitude coordinates
        print 'converting pond points to coordinate list'

        pond_dis.coordinate_list = pond_dis.dam_points_coordinates(pond_dis.pond_points)
        ...
        for p, i in zip(pond_dis.coordinate_list, range(len(pond_dis.coordinate_list))):
            ...
            pond = pond_dis.create_pond(dem=pond_dis.DEM,
                               flow_direction=pond_dis.FLOW_DIRECTION,
                               coordinates=p,
                               temp_point=pond_dis.temp_point)
